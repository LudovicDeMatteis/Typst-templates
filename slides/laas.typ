...
#let content_slide(
  ...
  slide[
    ...
    // Place the logos
    // LAAS
    #place(top + right, dx: 8%, dy: -15%, image(width: 20%, "LogoLAAS.png"))
  ]
}
...
#let slides(
  ...
  // Title slide
  slide[
    #place(dx: -8%, dy: 15%, block(height: 30%, width: 116%, inset: (left: 8%, right: 8%), fill: rgb("#001f3f"), text(
      fill: white,
      align(horizon + center)[
        #heading(level: 1, outlined: false, title)
      ],
    )))
    #place(dx: 50%, dy: 43%, block(
      fill: red,
      height: 4%,
      width: 60%,
    ))
    ...
    #block(inset: (left: 10%, right: 10%), grid(
    ...
    // Place the logos
    // LAAS
    #place(top + left, dx: -5%, dy: -12%, image(width: 20%, "LogoLAAS.png"))
    // CNRS
    #place(bottom + left, dx: 0%, dy: 5%, image(width: 8%, "LogoCNRS.png"))
    // UT
    #place(bottom + right, dx: 0%, dy: 10%, image(width: 20%, "LogoUT.png"))
  ]
